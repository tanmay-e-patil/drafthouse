...
pub async fn send_password_reset_email(to_email: &str, token: &str) -> Result<(), NanoServiceError> {
    let api_key = env::var("RESEND_API_KEY").unwrap_or_else(|_| "re_test_key".into());
    let app_origin = env::var("APP_ORIGIN").unwrap_or_else(|_| "http://localhost:3000".into());
...
<body style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p>If you didn't request this, you can safely ignore this email.</p>
    ...
    let from = env::var("EMAIL_FROM").unwrap_or_else(|_| "Drafthouse <onboarding@drafthouse.app>".into());
    ...
    let base_url = env::var("RESEND_API_BASE_URL").unwrap_or_else(|_| "https://api.resend.com".into());
